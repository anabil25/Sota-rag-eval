...
- [ ] PDF plugin: extract text per page, split by heading structure, convert to Markdown
...
- [ ] Streaming progress: subscribe to `session.on()` events, show `assistant.message_delta` for real-time progress in terminal
...
- [ ] Deduplication: detect near-duplicate questions across chunks (embedding similarity threshold)
...
- [ ] Curation UI page in web app (category-level controls, regenerate button)
- [ ] Copilot SDK streaming progress forwarded to UI
- [ ] Use `mode: "enqueue"` for queued regeneration
...
- [x] Bicep templates — modular structure:
  ...
  - (Not yet built: cosmos.bicep, functions.bicep, container-apps.bicep, postgresql.bicep)
...
- [ ] SOTA mode provisioner: per-variant index provisioning
...
- [x] Per-architecture index builder (`indexing/search_index.py`):
  ...
  - (Not yet built: multi-vector, agentic-KB, GraphRAG, LightRAG index builders)
...
- [ ] SOTA mode indexer: per-variant index creation
...
- [ ] Cost estimation: compute estimated monthly cost based on provisioned SKUs + query volume assumption
- [ ] Run against multiple architectures in parallel (async)
...
- [ ] Selection UI: from the web dashboard, mark architecture(s) to keep
...
- [ ] Live streaming of Copilot SDK events (message_delta, tool calls) to UI

### Deferred UI features (commented out, implement later)
- [ ] **Mode selection** (Step 3): Test Mode vs SOTA Mode toggle — needs to drive architecture selection and eval depth downstream. Currently hardcoded to Test Mode.
...
- [ ] **Eval curation** (Step 2): SME curation UI — steer category mix (more/fewer of certain categories), add/remove questions, regenerate with steering. Handler + API route exist but are commented out.
- [ ] **Teardown from UI**: Should appear on Provision or Compare page as a cleanup action (select architectures to tear down). Handler exists but is commented out. CLI: `retrieve teardown --keep hybrid`.
...
- [ ] Hooks integration:
  - `onPreToolUse` hook: log all tool calls to SQLite for audit trail
...
- [ ] Bicep validation: `az bicep build` on all templates in CI
...
- [ ] README.md: quickstart (ingest → eval → provision → run → compare), install, config reference
- [ ] Prerequisites doc: Copilot CLI installation (`winget install GitHub.Copilot` / `brew install copilot-cli`), `copilot login`, `az login`
- [ ] `retrieve.yaml` reference: all config fields including `model`, `provider` (BYOK), `otel`, architecture list, SOTA path toggles
- [ ] CLI reference: all subcommands with flags and examples
- [ ] Copilot SDK integration doc: how Retrieve uses the SDK, which tools are registered, how to configure BYOK, how to use Ollama locally
- [ ] Architecture registry docs: what each built-in architecture is, what it provisions
- [ ] SOTA path docs: each pre-defined path, what it includes, when to use it
- [ ] `pip install retrieve` packaging (or `pipx`) — list `github-copilot-sdk` as a dependency
- [ ] ~~Docker image~~ — **deferred post-v1** (complex multi-binary container, not needed for initial users)
...
- [ ] **Multi-vector (BGE-M3)**: Bicep for container instance (model server), index builder with dense + sparse + ColBERT fields, query executor with multi-vector fusion
- [ ] **Agentic retrieval (Knowledge Bases)**: Bicep for AI Search + Azure OpenAI + Azure Function orchestrator, Knowledge Base auto-ingest, KnowledgeAgent query with multi-hop reasoning
- [ ] **GraphRAG**: Bicep for Cosmos DB + Azure Functions + AI Search + Azure OpenAI, `graphrag` indexer integration (entity extraction, community detection), Global/Local/DRIFT query modes
- [ ] **LightRAG**: Bicep for AI Search + Azure OpenAI + graph store, `lightrag-hku` indexer integration, mix-mode query (graph + vector), lighter-weight alternative to full GraphRAG
- [ ] **SOTA eval runner**: Iterate toggle combinations from SOTA path registry, provision per-variant indexes, run eval set per-toggle, compute Δ deltas, feed into compare dashboard
...
- [ ] Eval-triggered fine-tuning recommendation: detect when retrieval scores plateau, suggest domain adaptation
...
- [ ] OpenTelemetry integration: COPILOT_OTEL_ENABLED, trace/cost/token tracking per eval run
- [ ] Skills packaging: prompt skills for eval-generate, failure-classify, eval-curate
- [ ] Docker image: CLI + web UI + Copilot CLI in one container (headless mode)
...
- [ ] CI/CD pipeline: GitHub Actions for lint + test + Bicep validation
