---
policy_id: "120-4"
title: "120-4 Fee Agent Training"
parent: "120 Fee Agents"
source_url: "C:\Users\nabilahmed\Documents\Code\alaska policy eval\policies\120\120-4_fee_agent_training.md"
last_ingested: "2026-03-31"
cross_references:
  - "{'id': '120-3', 'keywords': 'fee agent selection, application process, reference check'}"
  - "{'id': '120-5', 'keywords': 'billing, fee agent payments, master list'}"
---

Administrative Procedures Manual

[Click here to see this page in full context](#)

120-4          FEE AGENT TRAINING

Staff Development and Training (SD&T ) will provide the online training. Prior to being approved as a fee agent, the individual must take both the Fee Agent Training and HIPAAHealth Insurance Portability and Accountability Act Training at: <https://learn.dhss.alaska.gov/login/index.php>. They must complete the required trainings prior to interviewing applicants and submitting a monthly billing report to receive payment. Once DOSTDivision Operations Support Team confirms they have completed both trainings, the DPADivision of Public Assistance representative signs the Fee Agent Provider Agreement (FA 33Fee Agent Provider Agreement) approving the individual as a fee agent.

The fee agent manual available online explains the fee agent's roles and responsibilities. The manual is available on the DPAweb under Policy and Procedure Manuals <http://dpaweb.hss.state.ak.us/manuals/fee-agent/FeeAgentManual.pdf>.

By providing standardized training, fee agents will have thorough understanding of the following:

- The basic concept of the programs and services offered by DPADivision of Public Assistance;
- How to use the Fee Agent Manual;
- The role of fee agents in DPADivision of Public Assistance, their duties and responsibilities;
- Confidentiality rules;
- How to interview the applicant or recipient;
- How to complete the Fee Agent Interview Report (FA 1Fee Agent Interview Report);
- The various forms used in DPADivision of Public Assistance (application form, recertification form, and other forms);
- The requirements for verifying information; and
- The process of completing the Monthly Fee Agent Billing Report (FA 48Collecting Monthly Fee Agent Billing Report) form and how to bill the agency for payment.

|  |  |  |
| --- | --- | --- |
| [Previous Section](120-3_fee_agent_selection.md) |  | [Next Section](120-5_fee_agent_billing_and_payments.md) |
|  |  | [2020-02 (09/20)](../transmittals/transmittal_2020-02.md) |